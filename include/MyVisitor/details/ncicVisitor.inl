...
Ret Visitor<Ret (Impl::*)(const void*, Args...)>::Visit(std::size_t ID,
                                                        const void* ptr,
                                                        Args... args) {
  assert("ERROR::Visitor::Visit: unregistered" && IsRegistered(ID));
  auto target = impl_callbacks.find(ID);
  if (target != impl_callbacks.end())
    return target->second(details::Accessor<Impl>::template cast(this), ptr,
...
template <typename Impl, typename Ret, typename... Args>
template <typename T>
Ret Visitor<Ret (Impl::*)(const void*, Args...)>::Visit(const T* ptr,
                                                        Args... args) {
...
void Visitor<Ret (Impl::*)(const void*, Args...)>::RegisterOne() {
  impl_callbacks[details::CustomID<Derived>::get()] =
      [](Impl* impl, const void* ptr, Args... args) {
        return details::Accessor<Impl>::template run<const Derived*, Ret>(
            impl, ptr, std::forward<Args>(args)...);
      };
}

template <typename Impl, typename Ret, typename... Args>
bool Visitor<Ret (Impl::*)(const void*, Args...)>::IsRegistered(
    std::size_t ID) const {
