...
Ret Visitor<Ret(const void*, Args...)>::Visit(std::size_t ID, const void* ptr,
                                              Args... args) const {
...
template <typename Ret, typename... Args>
template <typename T>
Ret Visitor<Ret(const void*, Args...)>::Visit(const T* ptr,
                                              Args... args) const {
...
void Visitor<Ret(const void*, Args...)>::Register(std::size_t ID, Func&& func) {
  if constexpr (std::is_same_v<FuncTraits_Signature<Func>,
                               Ret(const void*, Args...)>) {
    ...
    callbacks[ID] = [func = std::forward<Func>(func)](const void* p,
                                                      Args... args) {
      return func(reinterpret_cast<const Derived*>(p),
...
void Visitor<Ret(const void*, Args...)>::RegisterOne(Func&& func) {
  ...
  callbacks[details::CustomID<RawDerived>::get()] =
      [func = std::forward<Func>(func)](const void* p, Args... args) {
        return func(reinterpret_cast<const RawDerived*>(p),
                    std::forward<Args>(args)...);
      };
}
